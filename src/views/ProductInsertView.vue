    ...
    <div>
        <form @submit.prevent="onSubmit" novalidate>
            ...
            <div style="margin: 10px">
                <button disabled v-if="isLoading">Saving...</button>
                <button type="submit" v-else>Save product</button>
            </div>
...
const validUrlRegex: RegExp = /^(https?:\/\/[a-zA-Z0-9\-.]+\.[a-zA-Z]{2,5}(?:\/\S*)?(?:[-A-Za-z0-9+&@#/%?=~_|!:,.;])+\.(?:jpg|jpeg|gif|png))$/g;

const productStore = useProductStore();
const { isLoading, errorMessage } = storeToRefs(productStore);
...
async function onSubmit(): Promise<void> {
    ...
    if (!v$.value.$error) {
        ...
        try {
            ...
            if (!errorMessage.value) router.push({ name: "products" });
        } catch (error) {
